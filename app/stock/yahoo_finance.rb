...
class YahooFinance
    ...
    def self.historical_quotes_save(symbol, options = {})
        options[:save] ||= true
        options[:savename] ||= :savename
        options[:period] ||= :daily
        read_historical(symbol, options)
    end
    ...
    def self.read_historical(symbol, options)
        params = {
        # warning: URI.escape is obsolete in ruby2.0
        # :s => URI.escape(symbol),
        :s => symbol,
        ...
        response = conn.read
        if options[:save]
            file = File.new(options[:savename], "w")
            file.puts response
            result = response
        else
            result = CSV.parse(response, :headers => cols) #:first_row, :header_converters => :symbol)
